...
#setwd("C:/Users/.../Aphareus furca") # If on a PC
#setwd("/Users/.../Aphareus furca") # If on a Mac
setwd("~/Desktop/KI_fish/analyses/SIMMs/Aphareus furca")
...
mix.filename  <- "~/Desktop/KI_fish/analyses/SIMMs/Aphareus furca/KI_fish_CSIA_C_AF.csv"
source.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Aphareus furca/KI_sources.csv"
discr.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Aphareus furca/KI_discrimination.csv"
...
mix_AF <- load_mix_data(filename=mix.filename, 
...
source <- load_source_data(filename=source.filename, source_factors=NULL, 
                           conc_dep=FALSE, data_type="means", mix_AF)

discr <- load_discr_data(filename=discr.filename, mix_AF)


### Write JAGS file
model_filename <- "MixSIAR_mix_AF.txt"
write_JAGS_model(model_filename, resid_err, process_err, mix_AF, source)

# MCMC run set to "test" for demonstration but was run as "very long" in final analyses
jags_AF <- run_model(run="test", mix_AF, source, discr, model_filename, 
                     alpha.prior = alpha.gen, resid_err, process_err)


### Analyze diagnostics and output
output_JAGS(jags_AF, mix_AF, source, output_options)

# Diagnostics output
diag <- output_diagnostics(jags_AF, mix_AF, source, output_options)
names(diag)
head(diag$gelman)
head(diag$geweke)

# Summary statistics 
df.stats <- output_stats(jags_AF, mix_AF, source, output_options)
...
df.stats3$species_code <- "AF"
df.stats3$sci_name <- "Aphareus furca"


### Save results
saveRDS(jags_AF, "mix_AF.rds")
#mix_AF <- readRDS("mix_AF.rds")
...
meta <- read.csv("KI_fish_CSIA_C_AF.csv") 
meta <- meta[,c(1:10)]
...
write.csv(df.stats2, "sum_stat_AF.csv")
write.csv(df.stats3, "sum_stat_AF_global.csv")
