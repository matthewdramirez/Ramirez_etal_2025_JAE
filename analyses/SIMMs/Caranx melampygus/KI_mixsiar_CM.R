...
#setwd("C:/Users/.../Caranx melampygus") # If on a PC
#setwd("/Users/.../Caranx melampygus") # If on a Mac
setwd("~/Desktop/KI_fish/analyses/SIMMs/Caranx melampygus")
...
mix.filename  <- "~/Desktop/KI_fish/analyses/SIMMs/Caranx melampygus/KI_fish_CSIA_C_CM.csv"
source.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Caranx melampygus/KI_sources.csv"
discr.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Caranx melampygus/KI_discrimination.csv"
...
mix_CM <- load_mix_data(filename=mix.filename, 
...
source <- load_source_data(filename=source.filename, source_factors=NULL, 
                           conc_dep=FALSE, data_type="means", mix_CM)

discr <- load_discr_data(filename=discr.filename, mix_CM)


### Write JAGS file
model_filename <- "MixSIAR_mix_CM.txt"
write_JAGS_model(model_filename, resid_err, process_err, mix_CM, source)

# MCMC run set to "test" for demonstration but was run as "very long" in final analyses
jags_CM <- run_model(run="test", mix_CM, source, discr, model_filename, 
                     alpha.prior = alpha.gen, resid_err, process_err)


### Analyze diagnostics and output
output_JAGS(jags_CM, mix_CM, source, output_options)

# Diagnostics output
diag <- output_diagnostics(jags_CM, mix_CM, source, output_options)
names(diag)
head(diag$gelman)
head(diag$geweke)

# Summary statistics 
df.stats <- output_stats(jags_CM, mix_CM, source, output_options)
...
df.stats3$species_code <- "CM"
df.stats3$sci_name <- "Caranx melampygus"


### Save results
saveRDS(jags_CM, "mix_CM.rds")
#mix_CM <- readRDS("mix_CM.rds")
...
meta <- read.csv("KI_fish_CSIA_C_CM.csv") 
meta <- meta[,c(1:10)]
...
write.csv(df.stats2, "sum_stat_CM.csv")
write.csv(df.stats3, "sum_stat_CM_global.csv")