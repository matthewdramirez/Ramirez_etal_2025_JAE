...
#setwd("C:/Users/.../Lutjanus bohar") # If on a PC
#setwd("/Users/.../Lutjanus bohar") # If on a Mac
setwd("~/Desktop/KI_fish/analyses/SIMMs/Lutjanus bohar")
...
mix.filename  <- "~/Desktop/KI_fish/analyses/SIMMs/Lutjanus bohar/KI_fish_CSIA_C_LB.csv"
source.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Lutjanus bohar/KI_sources.csv"
discr.filename <- "~/Desktop/KI_fish/analyses/SIMMs/Lutjanus bohar/KI_discrimination.csv"
...
mix_LB <- load_mix_data(filename=mix.filename, 
...
source <- load_source_data(filename=source.filename, source_factors=NULL, 
                           conc_dep=FALSE, data_type="means", mix_LB)

discr <- load_discr_data(filename=discr.filename, mix_LB)


### Write JAGS file
model_filename <- "MixSIAR_mix_LB.txt"
write_JAGS_model(model_filename, resid_err, process_err, mix_LB, source)

# MCMC run set to "test" for demonstration but was run as "very long" in final analyses
jags_LB <- run_model(run="test", mix_LB, source, discr, model_filename, 
                     alpha.prior = alpha.gen, resid_err, process_err)


### Analyze diagnostics and output
output_JAGS(jags_LB, mix_LB, source, output_options)

# Diagnostics output
diag <- output_diagnostics(jags_LB, mix_LB, source, output_options)
names(diag)
head(diag$gelman)
head(diag$geweke)

# Summary statistics 
df.stats <- output_stats(jags_LB, mix_LB, source, output_options)
...
df.stats3$species_code <- "LB"
df.stats3$sci_name <- "Lutjanus bohar"


### Save results
saveRDS(jags_LB, "mix_LB.rds")
#mix_LB <- readRDS("mix_LB.rds")
...
meta <- read.csv("KI_fish_CSIA_C_LB.csv") 
meta <- meta[,c(1:10)]
...
write.csv(df.stats2, "sum_stat_LB.csv")
write.csv(df.stats3, "sum_stat_LB_global.csv")
